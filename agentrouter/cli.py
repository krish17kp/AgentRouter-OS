...
from .engine import BASE_WEIGHTS, route as engine_route
...
app = typer.Typer(help="AgentRouter OS - route AI tasks to the best model/tool.",
...
def _load_registries():
    ...
    try:
        ...
        typer.echo("Next: fix the file above, or re-seed defaults with: agentrouter init --force",
                   err=True)
...
def _load_weights() -> dict:
    ...
    try:
        ...
        typer.echo("Next: fix the YAML, delete the file to use defaults, "
                   "or re-seed with: agentrouter init --force", err=True)
        raise typer.Exit(EXIT_REGISTRY)
    if not isinstance(cfg, dict):
        typer.echo(f"Config error: {cfg_path} must be a YAML mapping.", err=True)
        typer.echo("Next: delete the file to use defaults, or re-seed with: "
                   "agentrouter init --force", err=True)
        raise typer.Exit(EXIT_REGISTRY)
    w = cfg.get("weights") or {}
    valid = {k: v for k, v in w.items()
             if k in BASE_WEIGHTS and isinstance(v, (int, float))}
...
def _reason_for(rec: dict, cls, shifts: list[str]) -> str:
    ...
    t = rec["terms"]
    parts = [f"strongest weighted fit for {cls.task_type.value} "
...
def route(
    ...
    context_tokens: Optional[int] = typer.Option(None, "--context-tokens",
                                                 help="Override estimated context size."),
    risk: Optional[Level] = typer.Option(None, "--risk", help="Override inferred risk."),
    tool: Optional[str] = typer.Option(None, "--tool",
                                       help="Override tool needs (comma-separated)."),
...
def _print_route(task, cls, result, gates, reason, decision_id):
    ...
    typer.echo(f"  type: {cls.task_type.value:<14} complexity: {cls.complexity.value:<8}"
               f" risk: {cls.risk.value}")
    typer.echo(f"  context: ~{cls.context_tokens} tokens ({cls.context_band.value})"
    ...
    if result["recommendation"]:
        ...
        typer.echo("  Why: every registry model was excluded by a hard filter"
                   " (context size, required tools, or retirement).")
        typer.echo("  Next: relax --tool/--context-tokens overrides, or add a capable model to"
    ...
    if decision_id:
        typer.echo(f"\nDecision logged: {decision_id}"
                   f"  (prompt saved with it; task text is stored locally)")
        typer.echo(f"Next: agentrouter explain {decision_id}"
...
def explain(
    ...
    if not db_path.exists():
        ...
        typer.echo('Next: agentrouter init (if not done), then agentrouter route "<task>"', err=True)
    ...
    typer.echo(f"Classification: {c['task_type']}/{c['complexity']}/{c['risk']}/"
    ...
    for i, row in enumerate(payload["scores"]):
        ...
        mark = "  <- recommended" if i == 0 else (
            "  <- fallback" if payload["fallback"] and row["model"] == payload["fallback"]["model"] else "")
        typer.echo(f"  {row['model']:<42} {t['cap']:>5} {t['cost']:>5} {t['lat']:>5}"
    ...
    g = payload["gates"]
    typer.echo(f"Gate: approval={g['approval_level']}, auto_execute_allowed={g['auto_execute_allowed']}")


# --- feedback (Advanced tier — stub) ---------------------------------------------

@app.command()
def feedback(decision_id: str, rating: int = typer.Option(..., "--rating", min=1, max=5),
    ...
    conn.execute("INSERT INTO feedback (decision_id, created_at, rating, note) VALUES (?, ?, ?, ?)",
                 (int(decision_id.removeprefix("d_")),
                  datetime.now(timezone.utc).isoformat(), rating, note))
    conn.commit()
    conn.close()
    typer.echo(f"Recorded rating {rating} for {decision_id}. "
...
def registry_list(
    ...
    if json_out:
        ...
        return
    typer.echo(f"{'PROVIDER':<12} {'MODEL_ID':<28} {'CTX':>10} {'PRICE':<9} {'LAT':<7}"
               f" {'C/R/W':<7} TOOLS")
    for m in rows:
        crw = f"{m.ability.coding}/{m.ability.reasoning}/{m.ability.writing}"
        typer.echo(f"{m.provider:<12} {m.model_id:<28} {m.context_window:>10}"
...
def providers_refresh(
    ...
    limit: int = typer.Option(25, "--limit", min=1,
                              help="Max models to import from the catalog."),
    ...
    if provider not in SUPPORTED_PROVIDERS:
        ...
        typer.echo(f"Why: only these adapters have live refresh: "
                   f"{', '.join(SUPPORTED_PROVIDERS)}.", err=True)
        typer.echo(f"Next: agentrouter providers refresh openrouter, or edit "
                   f"{_home() / 'registry' / 'models.yaml'} by hand.", err=True)
        raise typer.Exit(EXIT_USAGE)
    ...
    typer.echo("auth: using OPENROUTER_API_KEY from environment" if api_key
    ...
    try:
        ...
        typer.echo("Why: the live catalog could not be fetched or parsed; your "
                   "registry was NOT modified.", err=True)
        typer.echo("Next: check your network, retry later, or keep using the manual "
                   "registry (models.yaml) - routing works without refresh.", err=True)
    ...
    typer.echo(f"\nWrote {path}")
    typer.echo("Manual models.yaml is untouched and wins on collision. "
...
def prompt_generate(
    task: Optional[str] = typer.Argument(None, help="Task text (or use --from)."),
    from_id: Optional[str] = typer.Option(None, "--from", help="Regenerate from a decision id."),
    tool: Optional[str] = typer.Option(None, "--tool", help="Target tool key (provider/model_id)."),
    out: Optional[Path] = typer.Option(None, "--out", help="Write prompt to a file."),
):
    ...
    if from_id:
        ...
        target = tool or (payload["recommendation"]["model"] if payload["recommendation"]
